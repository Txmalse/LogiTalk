...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.label = None
        #loading img
        self.send_img = CTkImage(light_image = Image.open('assets/icons/paper-plane.png'), size=(20,20))
        self.open_menu_img = CTkImage(light_image=Image.open('assets/icons/menu-burger.png'), size=(20, 20))
        self.close_menu_img = CTkImage(light_image=Image.open('assets/icons/cross.png'), size=(20, 20))
        # menu frame
        self.menu_frame = CTkFrame(self, width=36, height=300)
        self.menu_frame.pack_propagate(False)
        self.menu_frame.place(x=0, y=0)
        self.is_show_menu = False
        self.speed_animate_menu = -5
        self.btn = CTkButton(self, text='',image= self.open_menu_img, command=self.toggle_show_menu, width=30)
    ...
    def toggle_show_menu(self):
       if self.is_show_menu:
           ...
           self.btn.configure(image=self.open_menu_img)
           self.show_menu()
       else:
           self.is_show_menu = True
           self.speed_animate_menu *= -1
           self.btn.configure(image= self.close_menu_img)
           self.show_menu()
           # налаштування меню
    ...
    def show_menu(self):
        ...
        elif self.menu_frame.winfo_width() >= 46 and not self.is_show_menu:
            self.after(10, self.show_menu)
